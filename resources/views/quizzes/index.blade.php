...
<div class="flex gap-3 text-sm text-white/80 mt-2">
    <span>{{ ['arabic' => '🌍 العربية', 'english' => '🌎 الإنجليزية', 'hebrew' => '🌏 العبرية'][$quiz->subject] ?? '🌐 غير محدد' }}</span>
    <span>📚 الصف {{ $quiz->grade_level ?? 'غير محدد' }}</span>
</div>
                            </div>
...
<a href="{{ route('results.quiz', $quiz->id) }}" class="mt-2 block w-full bg-purple-500 hover:bg-purple-600 text-white px-3 py-2 rounded-lg text-center transition">
